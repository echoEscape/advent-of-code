#Execution results in a text File with various numbers. Adding them all together results in the correct number for the task.
#Do not ask me how it worked. The recursion melted my brain and that was the only way I managed to cancel it without an infinity loop.
#Reminder @ myself: Never use this in a productive setting.

rawData = """dotted blue bags contain 5 wavy green bags, 3 pale beige bags.
...
rawData = """dark maroon bags contain 2 striped silver bags, 4 mirrored maroon bags, 5 shiny gold bags, 1 dotted gold bag.
...
bagRules = rawData.split("\n")
mainBags = []
subBags = []

for i in range(len(bagRules)):
    tempSplit = bagRules[i].split(" contain ")
    mainBags.append(tempSplit[0])
    #print(tempSplit[0])
    subBags.append(tempSplit[1].split(", "))

bagList = list(zip(mainBags, subBags))

for i in range(len(bagList)):
    for k in range(len(bagList[i][1])):
        #print(bagList[i][k])
        if(bagList[i][1][k].endswith(".")):
            bagList[i][1][k] = bagList[i][1][k][:-1]
        if(bagList[i][1][k].endswith("s") == False):
            bagList[i][1][k] += "s"
...
#Get subbags -> put in array/list
#add amount to global counter
#call function again
#iterate through new subbags
#add amount to global counter
#call function again
#iterate through new subbags
#...

#Get ruleset for shiny gold bags
startPoint = []
for i in range(len(bagList)):
    if bagList[i][0] == "shiny gold bags":
        startPoint = bagList[i]

def findRuleset(search):
    for i in range(len(bagList)):
        if bagList[i][0] == search:
            return bagList[i]

globalCnt = 0
testCount = 0
def findAllBagsInside(startPoint, bagList, globalCnt):
    queue = []
    amount = []
    for i in range(len(startPoint[1])):
        tempSub = startPoint[1][i].split(" ", 1)
        if tempSub[0] != "no":
            globalCnt += int(tempSub[0])
            queue.append(tempSub[1])
            amount.append(int(tempSub[0]))

    for i in range(len(queue)):
        newStartPoint = findRuleset(queue[i])
        for k in range(amount[i]):
            findAllBagsInside(newStartPoint, bagList, globalCnt)

    print(globalCnt)
    f.write(str(globalCnt) + "\n")

findAllBagsInside(startPoint, bagList, globalCnt)
#print(globalCnt)
f.close()

#Ansatz Teil 1
'''def bfs(visited, bagList, search):
    ...
    while queue:
        ...
        for i in range(len(bagList)):
            ...
            #print(bagList[i][1])
            for k in range(len(bagList[i][1])):
                tempString = bagList[i][1][k]
                tempString = tempString.split(" ", 1)
                #print(tempString[1])

                if(s == tempString[1]):
                    visited.append(bagList[i][0])
                    queue.append(bagList[i][0])

#print(bagList[0][0])
bfs(visited, bagList, "shiny gold bags")
print(len(visited))'''
#------------------------


#Aufbereiten der untertaschen
'''for i in range(len(subBags)):                               #Gruppe Sub-Bags
    tempSub = []
    tempType = []
    tempSubBag = [[]]

    for n in range(len(subBags[i])):                        #Einzelne Sub-Bags
        if(subBags[i][n].endswith(".")):
            subBags[i][n] = subBags[i][n][:-1]
        if(subBags[i][n].endswith("s") == False):
            subBags[i][n] += "s"

        words = subBags[i][n].split(" ")
        if(words[0] == "no"):
            amountBags = 0
            typeBag = "None"
        else:
            amountBags = int(words[0])
            typeBag = words[1] + words[2] + words[3]

        #amountBags und typeBags zu Array[i] 
    tempSubBag[i] = typeBag
    tempSubBag.append(amountBags)
    print(amountBags)
    print(typeBag)'''
#print(subBags)
'''def bfs(visited, mainBags, subBags, search):
    visited.append(search)
    queue.append(search)

    while queue:
        s = queue.pop(0)
        #print(s, end = " ")    #Print the removed queue entry/search

        for i in range(len(mainBags)):
            if any(s in k for k in subBags[i]):
                visited.append(mainBags[i])
                queue.append(mainBags[i])

for i in range(len(bagRules)):
    tempGroup = []
    tempRules = bagRules[i].split("contain", 1);
    mainBags[i] = tempRules[0][:-2]         #Only name of mainBags are there
    tempSubBags = tempRules[1][1:]          #SubBags are still in one string
    tempSubBags = tempSubBags.split(",")    #Split current SubBag string

    for n in range(len(tempSubBags)):
        tempGroup.append(tempSubBags[n].strip())    #Remove empty spaces at start and end
        subBags[i] = tempGroup

bfs(visited, mainBags, subBags, "shiny gold bags")
final_visited = list(dict.fromkeys(visited))
...
'''for i in range(len(bagRules)):
    tempGroup = []
    tempRules = bagRules[i].split("contain", 1);
    mainBags[i] = tempRules[0][:-2]
    tempSubBags = tempRules[1][1:]

    tempSubBags = tempSubBags.split(",")
    for n in range(len(tempSubBags)):
        tempGroup.append(tempSubBags[n].strip())
    #subBags[i] = tempRules[1][1:]
    print(tempGroup)
    allBags = map(mainBags[i], tempGroup)
...
def findBags(search, queue):
    for i in range(len(mainBags)):
        if subBags[i].find(search) != -1:
            tempBags.append(mainBags[i])
            queue.append(mainBags[i])
    queue.remove(search)

    if len(queue) != 0:
        for i in range(len(queue)):
            findBags(queue[i], queue)

#findBags("shiny gold bag", queue)
#print(tempBags)'''

'''for i in range(len(subBags)):
    subBags[i].append(subBags[i].split(","))
    if subBags[i].find("shiny gold bag") != -1:
        print(subBags[i])'''

'''
Function which:
takes searchword
adds +1 if searchword found
adds main to list
...
'''tempList = []
currentBagLen = 0
def findBags(mainBags, subBags, typeBag, tempBagLen):
    currentBagLen = tempBagLen                  #Break out of recursion if it doesnt change

    for i in range(len(mainBags)):
        if subBags[i].find(typeBag) != -1:
            #bag_cnt += 1
            tempList.append(mainBags[i])
            tempBagLen += 1

    if tempBagLen != currentBagLen:
        for i in range(len(tempList)):
            findBags(mainBags, subBags, tempList[i], tempBagLen)

findBags(mainBags, subBags, "shiny gold", currentBagLen)
print(tempList)
def splitSubBags(subBags, typeBag):
    bag_cnt = 0
    for i in range(len(subBags)):
        if subBags[i].find(typeBag) != -1:
            bag_cnt += 1
            posBag = subBags[i].find(typeBag)
            #print(subBags[i][:posBag])
            tempNum = subBags[i][:posBag].split(",")
            tempNum = tempNum[len(tempNum)-1]
            tempList.append(int(tempNum))
    return "aaa", bag_cnt

includesShinyGold = []
includesShinyGold.append(splitSubBags(subBags, "shiny gold bag"))


#tempAmount = []
#tempAmount.append(splitSubBags(subBags))
#print(tempAmount)'''