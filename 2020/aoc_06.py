...
teamList = rawList.split("\n\n")
memberList = ["" for i in range(len(teamList))]
answerList = ["" for i in range(len(teamList))]
answer_cnt = 0
for i in range(len(teamList)):
    memberList[i] = teamList[i].split()

for i in range(len(memberList)):                 #grab entire group
    tempAnswer = ""
    for answer in memberList[i]:
        for k in range(len(answer)):
            if answer[k] not in tempAnswer:
                tempAnswer = tempAnswer + answer[k]
    answerList[i] = tempAnswer

for i in range(len(answerList)):
    tempNum = len(answerList[i])
    answer_cnt = answer_cnt + int(tempNum)

#Part 2
tempChar = ""
for i in range(len(answerList)):
    tempAnswer = answerList[i]              #Full AnswerStr -> gets changed in the if below
    for k in range(len(answerList[i])):     #get Len of AnswerString -> k gives you the letter
        tempChar = answerList[i][k]
        
        #Iterate through all answers of a group
        for answer in memberList[i]:
            if tempChar not in answer:
                tempAnswer = tempAnswer.replace(tempChar, '')
    answerList[i] = tempAnswer

answer_cnt = 0
for i in range(len(answerList)):
    tempNum = len(answerList[i])
    answer_cnt = int(answer_cnt) + int(tempNum)
print(answer_cnt)