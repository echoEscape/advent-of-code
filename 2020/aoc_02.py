...
pwArr = []      #rawArray
typeList = []   #MD-array split in limit, letter, pw

def stringSplit(rawList):
    def splitSpace():
        tempTypeList = []
        for i in range(len(lineList)):
            tempResult = lineList[i].split()
            tempTypeList.append(tempResult)
        return tempTypeList

    lineList = rawList.splitlines()
    typeList = splitSpace()

    return typeList

def checkValidity(rawArr):
    limiter =  [[0 for x in range(len(rawArr))] for y in range(len(rawArr))]
    testChar = ""
    validPws = 0
    for i in range(len(rawArr)):        #Go through entire array
        limiter[i] = rawArr[i][0].split("-")
        ...
        #Task1
        '''amountChars = password.count(testChar)'''
        
        #Task2
        minChar = int(limiter[i][0])-1
        maxChar = int(limiter[i][1])-1

        if password[minChar] == testChar and not password[maxChar] == testChar or password[maxChar] == testChar and not password[minChar] == testChar:
            validPws = validPws+1

        #Task1
        '''if amountChars >= int(limiter[i][0]) and amountChars <= int(limiter[i][1]):
            validPws = validPws+1
            print("valid")
        else:
            print("invalid")'''
